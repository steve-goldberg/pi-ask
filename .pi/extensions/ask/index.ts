...
export default function askExtension(pi: ExtensionAPI) {
  pi.registerCommand("ask", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!ctx.isIdle()) {
        ctx.ui.notify("/ask can only run when pi is idle.", "warning");
        return;
      }
      ...
      try {
        ...
        const resolution = await resolveQuestionnaireDefinition(ctx, {
          ...
          artifacts,
          fallbackDefinition: DEFAULT_ASK_QUESTIONNAIRE,
        });

        ctx.ui.notify(`Opening /ask. ${formatQuestionnaireProvenance(resolution.provenance)}`, "info");
        ...
        if (result.status === "cancelled") {
          ctx.ui.notify("/ask cancelled. Draft kept in .pi/tmp/ask.json.", "info");
          return;
        }
        ...
        try {
          ...
          ctx.ui.notify(
            `/ask finished but could not hand answers back into the session. Draft kept in .pi/tmp/ask.json. ${handoffMessage}`,
            "error",
          );
          return;
        }
        ...
        try {
          ...
          ctx.ui.notify(
            "/ask answers submitted, but the draft file could not be removed.",
            "warning",
          );
          return;
        }

        ctx.ui.notify("/ask answers submitted to the active session.", "info");
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        ctx.ui.notify(`/ask failed: ${message}`, "error");
      }
    },
  });
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use ask when you need multiple clarification answers from the user and a one-question-at-a-time TUI flow would be better than writing a long question block in chat.",
    ...
    async execute(_toolCallId, params, _signal, onUpdate, ctx) {
      if (!ctx.hasUI) {
        throw new Error("ask requires interactive TUI mode.");
      }
      ...
      if (result.status === "cancelled") {
        return {
          ...
          } satisfies AskToolResultDetails,
        };
      }
